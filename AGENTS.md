...
- `h5/`: H5 版「朱迪单词」（吉祥物为疯狂动物城的朱迪兔），面向四年级小学生的零依赖纯静态学习游戏。复用 RAZ 词库（AA–Z2 全 29 级内嵌）、Boss 战参数（15 题/120s/85%/48h 冷却，首次弃战豁免）、SM-2 间隔重复与有道发音 API；含定级测试（起点 D，区间 AA–H）、五题型课程引擎（词义/反选/听音/拼写/例句填空，填空仅用「目标词恰一次独立出现」的精语料句）、错题重排队、每日目标、每日打卡（+1 星、连续 7/14/30 天里程碑加奖）、徽章系统、进度导出/导入；进度存 localStorage。
