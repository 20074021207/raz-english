...
## [2026-09-18] 两处真机 bug 根治：didStart/didFinish 全哑 + ❌ 多层冗余

**根因（研读代码确认）**:
1. 焦点框停在第三句 + 之前的 10 秒慢节奏，同源：WebViewController 从未设置
   `synthesizer.delegate = self`——AVSpeech 的 didStart/didFinish 全部不触发；
   逐句进度实际由"入队定时器"在 0.24s 内连发 0→1→2→3，高亮直接跳第三句停死。
2. ❌ 无响应：网页端链路正常，设备端疑似事件绑定时机/冒泡问题。

**修复**:
- Swift：loadView 补 synthesizer.delegate = self；进度回调只由 didStart 驱动
  （真正开口才推进），另按估时排单调兜底链（startedIndices 抑制重复）；
  didFinish 正常接管完成回调，节奏由真实朗读时长驱动。
- JS：原生回调统一分发器（单句/整组共用，id 校验+单调守卫，乱序/迟到忽略）；
  分发器移入 NG.audio 对象（修复顶层赋值在对象创建前抛错导致模块加载失败）。
- ❌ 多层冗余：直接绑定（每屏渲染时）+ 全局 click 委托 + touchend 委托；
  confirmExit 防重入（modal 存在即跳过）；委托异常兜底直接回首页。
- 桩桥验证：高亮逐句推进（word→句1→句2）、乱序回调被守卫忽略、三种 ❌
  路径（直接/委托/克隆按钮/touchend）全部生效；E2E 全过零错误；重装真机 100%。

## [2026-09-19] 例句改离线文件播放 + 测验全题型朗读 + 重新定级入口

**背景**: 用户反馈 iOS App 例句仍无声（AVSpeech 实时输出在设备上静音），
且希望测验时读单词、可重新定级。实测有道/百度/fanyi 整句 TTS 端点全部不可用，
确定离线渲染为唯一可靠方案。

**修复**:
- 原生层主通路重构：AVSpeech write() 把例句渲染为 CAF 文件（磁盘缓存，DJB2 命名）
  → AVAudioPlayer 播放——与语音合成完全不同的普通文件播放通路，可靠出声；
  渲染串行 + 12s 防挂起；播放链句间 1 秒，AVAudioPlayerDelegate 推进；
  渲染失败回退实时合成，再失败 JS 防挂起兜底，三层不锁死。
- 测验全题型（课程+Boss）出题即朗读单词（cloze 除外——目标词即答案不泄露）；
  作答后无论对错朗读单词强化（此前仅答对读）。
- 首页冒险地图卡内新增「🧭 重新测评定级」入口（原仅藏于设置弹窗）。
- E2E 全过；重装真机 100%。
