...
// 复刻 audit-corpus.mjs 的精确校验逻辑，仅针对 batch-24 + 跨批次查重
...
const batch = JSON.parse(readFileSync(dir + 'batch-24.json', 'utf8'));
const wordList = JSON.parse(readFileSync(dir + 'words-batch-24.json', 'utf8'));
...
for (const f of readdirSync(dir)) {
  if (!/^batch-\d+\.json$/.test(f) || f === 'batch-24.json') continue;
