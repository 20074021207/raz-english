...
// 复刻 audit-corpus.mjs 的精确校验逻辑，仅针对 batch-39 + 跨批次查重
...
const batch = JSON.parse(readFileSync(dir + 'batch-39.json', 'utf8'));
const wordList = JSON.parse(readFileSync(dir + 'words-batch-39.json', 'utf8'));
...
if (missing.length) console.log('MISSING KEYS (dropped words):', missing.join(', '));
if (extra.length) console.log('EXTRA KEYS:', extra.join(', '));

// 跨批次英文句查重 + 词键重复检查（跳过本批次）
...
for (let i = 1; i <= 60; i++) {
  ...
  for (const [w, sents] of Object.entries(data)) {
    if (batchKeys.includes(w)) dupWords.add(w);
    for (const [en] of sents) if (!seenEn.has(en.toLowerCase())) seenEn.set(en.toLowerCase(), w);
  }
}
if (dupWords.size) console.log('DUP WORD KEYS in other batches:', [...dupWords].join(', '));
