...
// 复刻 audit-corpus.mjs 的精确校验逻辑，仅针对 batch-37 + 跨批次查重
...
const batch = JSON.parse(readFileSync(dir + 'batch-37.json', 'utf8'));
const wordList = JSON.parse(readFileSync(dir + 'words-batch-37.json', 'utf8'));
...
const errors = [];
console.log('entries:', batchKeys.length, '| wordlist:', wordList.length, '| dropped by author:', wordList.length - batchKeys.length);
for (const [word, sents] of Object.entries(batch)) {
  ...
  const isPhrase = t.includes(' ') || t.includes('-');
